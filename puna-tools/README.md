...
`--bin puna-orchestrator` by name, so nothing here reaches an image. Both are still built, linted
...
`--locations` is the only size knob. Items and locations are the same number by construction (each
game's item table is `--locations` names, being `--locations - 1` ordinary ones plus `Goal`),
...
played by `room-load` reach an end, as below. It also embeds **`collect_mode: "disabled"`**,
...
resembling play. Termination is unaffected, since that rests on auto-release.
...
unfinished world at once, which is the biggest `to_slot` burst a room ever produces, and closing at
the moment of the last goal would measure everything except it.
...
half a second doubling to a thirty-second ceiling and jittered, and keeps trying for as long as the
run is unfinished. Nothing else about the slot restarts: the to-send list comes from the room's own
...
clients return**, which is the half of backpressure a tool that gave up was never exercising. And a
...
received on a 200-slot room, or 1.2 GB across a few runs, uncompressed. The room drops the connections
